...
watcher 不属于 SDK。POSIX/Windows CLI 用 polling + mtime + debounce 检测
`config.json` 或 skill 文件变化，再调用 runtime builder reload。ESP 可以保留静态
skill catalog，同时不编译 watcher 和桌面文件监听逻辑。

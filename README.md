...
```bash
./build/posix-cli/apps/posix/cli/c-claw --help
./build/posix-cli/apps/posix/cli/c-claw
./build/posix-cli/apps/posix/cli/c-claw ask "列出当前工作区文件"
```
...
插件配置归属具体应用。POSIX CLI 默认路径是
`apps/posix/cli/config/plugins.json`；ESP32-S3 QEMU profile 不编入外部进程插件。

`apps/posix/cli/config/plugins.json` 示例：
...
| 插件工具 | `CC_ENABLE_PLUGIN` | 由 `plugins.json` 注册 |

...
apps/       应用和板级组合：POSIX CLI、STM32MP135 board、进程型 plugin、MCP transport
build/app/  应用或板级构建产物
...
STM32MP135 board      build/app/posix/stm32mp135_board  c-claw-board
```
...
旧的实验 app 已移除；当前正式 app 入口集中在 POSIX CLI 和
STM32MP135 board。面试或复盘时要以当前仓库实际入口为准，不要把已删除的实验入口
说成仍在维护。
...
apps/                   ← 应用入口（约 40+ 个 C/H 文件）
  ...
  posix/stm32mp135_board/ ← STM32MP135 board（gateway、feature、plugin、mcp、sandbox、board tools）
```
...
| 构建 Profile | 支持 POSIX CLI、core-minimal、STM32MP135 board 3 类构建目标 | 体现跨平台和裁剪 |
| 自动化测试 | POSIX CLI 和 STM32MP135 board 的 CTest 全部通过 | 体现工程质量 |
| 板级应用 | STM32MP135 board 提供摄像头、音频、CAN、ADC 工具入口 | 体现不是纯桌面 demo |
| 插件协议 | JSON-RPC 2.0 over stdio，插件可用任意语言实现 | 体现扩展性 |

简历里推荐组合成一句：

> 基于纯 C 实现 AI Agent Runtime，采用 Ports & Adapters 架构解耦 LLM、工具、存储和平台能力，支持 3 类 LLM 后端、3 类存储后端、JSON-RPC 插件工具系统，并通过 CMake Profile 覆盖 POSIX CLI、core-minimal 和 STM32MP135 board 等构建目标；编写 CTest 覆盖并发队列、插件协议、工具执行和配置加载等核心路径。
...
| STM32MP135 board 二进制体积 | 板级部署方向很加分 | `c-claw-board` 单文件产物约 xx MB |
...
> 通过 CMake Profile 支持 POSIX CLI、core-minimal、STM32MP135 board 等裁剪构建，编写 CTest 覆盖并发调度、插件协议、工具执行和配置加载等关键路径。
...
> 通过 CMake Profile 和平台端口实现同一套 Agent Core 在 POSIX CLI、core-minimal 和 STM32MP135 board 之间裁剪复用；板级 app 只保留所需硬件工具和运行时能力，桌面能力不进入核心层。
...
**追问补充**：我不会把已删除的实验入口说成当前维护目标；当前验证口径以 POSIX CLI、
core-minimal 和 STM32MP135 board 为准。
...
构建目录按层分开：桌面应用使用 `build/app/posix/cli`，SDK 最小裁剪使用
`build/sdk/core-minimal`，STM32MP135 board 使用 `build/app/posix/stm32mp135_board`。
这样阅读 build 产物时能直接看出它属于

...
int main(void)
{
    ...
    return 0;
#else
    char *enabled[] = { "read", "camera", "audio", "can", "adc" };
    cc_config_t config;
    ...
    config.enabled_tools_count = sizeof(enabled) / sizeof(enabled[0]);
    ...
    for (size_t i = 0; i < name_count; i++) {
        ...
        if (names[i] && strcmp(names[i], "board.camera") == 0) found_camera = 1;
        if (names[i] && strcmp(names[i], "board.audio") == 0) found_audio = 1;
        if (names[i] && strcmp(names[i], "board.can") == 0) found_can = 1;
        if (names[i] && strcmp(names[i], "board.adc") == 0) found_adc = 1;
    }
    if (count < 5 || name_count < 5 || !found_read || !found_camera ||
        !found_audio || !found_can || !found_adc) {
        failed = 1;
    }
